...
START_TEST(test_sc_ecb_dec) {
    ...
    sc_dec_ecb(c, b, len, k);
    ck_assert_int_eq(memcmp(b, m, len), 0);
}
END_TEST
...
gdb_suite(void)
{
    ...
    tcase_add_test(tc_scrypt, test_sc_ecb_dec);
    tcase_add_test(tc_scrypt, test_sc_cbc);
    tcase_add_test(tc_scrypt, test_sc_cbc_dec);
